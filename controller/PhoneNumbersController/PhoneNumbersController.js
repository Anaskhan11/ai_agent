...
const listPhoneNumbers = async (req, res) => {
  try {
    ...
    console.log("👤 User ID:", currentUserId, "🛡️ Super Admin:", isSuperAdmin);
...
const createPhoneNumber = async (req, res) => {
  try {
    ...
    switch (provider.toLowerCase()) {
      case 'twilio':
        ...
        console.log('Twilio credentials debug:', {
          ...
          twilioAccountSidType: typeof twilioAccountSid,
          twilioAuthToken: twilioAuthToken,
          twilioAuthTokenType: typeof twilioAuthToken
        });
    ...
    // Deduct credits for phone number creation (only if not super admin)
    if (req.creditInfo && !req.creditInfo.bypassCredits) {
      try {
        await deductCredits(req, null, () => {});
        console.log('✅ Credits deducted for phone number creation');
      } catch (creditError) {
        console.error('❌ Error deducting credits for phone number creation:', creditError);
        // Note: We don't fail the phone number creation if credit deduction fails
        // as the phone number has already been created successfully
      }
    }
...
const importTwilioPhoneNumber = async (req, res) => {
  try {
    console.log('🚀🚀🚀 IMPORT TWILIO FUNCTION CALLED - NEW VERSION - UPDATED');
    const currentUserId = req.user?.user?.id || req.user?.id;
    ...
    if (!currentUserId) {
      return res.status(401).json({
        success: false,
        message: "Authentication required. Please log in to import phone numbers."
      });
    }
    ...
    if (!twilioPhoneNumber || !twilioAccountSid || !twilioAuthToken) {
      return res.status(400).json({
        success: false,
        message: "Twilio phone number, account SID, and auth token are required"
      });
    }
    ...
    // Prepare data for VAPI import Twilio API
    const importData = {
    ...
    if (twilioLabel) {
      importData.name = twilioLabel;
    }

    if (assistantId) {
      importData.assistantId = assistantId;
    }

    console.log('🔥 IMPORTING TWILIO PHONE NUMBER WITH CORRECT DATA:', JSON.stringify(importData, null, 2));

    // Initialize VAPI service
    const vapiService = new VapiService();

    // Import phone number using VAPI import API
    const vapiPhoneNumber = await vapiService.importTwilioPhoneNumber(importData);
    ...
    const phoneNumberRecord = {
      user_id: currentUserId,
      phone_number_id: vapiPhoneNumber.id,
      org_id: vapiPhoneNumber.orgId || null,
      number: vapiPhoneNumber.number || twilioPhoneNumber,
      country_code: vapiPhoneNumber.countryCode || null,
      provider: 'twilio',
      type: 'local',
      capabilities: vapiPhoneNumber.capabilities || {},
    ...
    const phoneNumberId = await PhoneNumberModel.createPhoneNumber(phoneNumberRecord);

    console.log('✅ Twilio phone number imported successfully:', phoneNumberId);

    // Get the created phone number record
    const createdPhoneNumber = await PhoneNumberModel.getPhoneNumberById(phoneNumberId);
    ...
    res.status(201).json({
      ...
      phoneNumber: {
        ...
        provider: 'twilio',
        label: twilioLabel || 'Imported Twilio Number',
    ...
    // Check if the error is about phone number already being claimed
    if (error.message && error.message.includes("already in use")) {
      return res.status(400).json({
        success: false,
        message: "Phone number already claimed",
        error: "This phone number is already registered with another organization. Please use a different phone number."
      });
    }

    res.status(500).json({
      success: false,
      message: "Failed to import Twilio phone number",
      error: error.message
    });
  }
};
...
// Get all phone numbers for super admin (shows all users' phone numbers)
const getAllPhoneNumbersForAdmin = async (req, res) => {
  try {
    const currentUserId = req.user?.user?.id || req.user?.id;
    const isSuperAdmin = req.user?.isSuperAdmin || false;
    ...
    if (!isSuperAdmin) {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only super admin can view all phone numbers."
      });
    }

    const PhoneNumberModel = require("../../model/PhoneNumberModel/PhoneNumberModel");
    const phoneNumbers = await PhoneNumberModel.getAllPhoneNumbersForAdmin();
...
const getAllVapiPhoneNumbersForAdmin = async (req, res) => {
  try {
    const currentUserId = req.user?.user?.id || req.user?.id;
    const isSuperAdmin = req.user?.isSuperAdmin || false;
    ...
    if (!isSuperAdmin) {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only super admin can view all VAPI phone numbers."
      });
    }

    console.log("🔍 Fetching all VAPI phone numbers for admin...");

    // Step 1: Get all phone numbers from VAPI
    const response = await axios.get(`${VAPI_BASE_URL}/phone-number`, {
      headers: {
        Authorization: `Bearer ${VAPI_SECRET_KEY}`,
        "Content-Type": "application/json"
    ...
    let vapiPhoneNumbers = response.data || [];
    console.log(`📱 Found ${vapiPhoneNumbers.length} phone numbers in VAPI`);

    // Step 2: Get local assignment data
    const PhoneNumberModel = require("../../model/PhoneNumberModel/PhoneNumberModel");
    const localPhoneNumbers = await PhoneNumberModel.getAllPhoneNumbersForAdmin();

    // Step 3: Create a map of local assignments by VAPI phone_number_id
    const assignmentMap = new Map();
    localPhoneNumbers.forEach(localPn => {
      if (localPn.phone_number_id) {
        assignmentMap.set(localPn.phone_number_id, {
          local_id: localPn.id,
          user_id: localPn.user_id,
          user_name: localPn.user_name,
          user_email: localPn.user_email,
          first_name: localPn.first_name,
          last_name: localPn.last_name,
          created_at: localPn.created_at,
          updated_at: localPn.updated_at
        });
      }
    });

    // Step 4: Merge VAPI data with local assignment data
    const enrichedPhoneNumbers = vapiPhoneNumbers.map(vapiPn => {
      const assignment = assignmentMap.get(vapiPn.id);
      return {
        // VAPI data
        id: vapiPn.id, // This is the VAPI ID
        vapi_id: vapiPn.id,
        number: vapiPn.number,
        provider: vapiPn.provider || 'vapi',
        country_code: vapiPn.countryCode,
        type: vapiPn.type,
        capabilities: vapiPn.capabilities,
        cost: vapiPn.cost,
        // Local assignment data
        local_id: assignment?.local_id || null,
        user_id: assignment?.user_id || null,
        user_name: assignment?.user_name || null,
        user_email: assignment?.user_email || null,
        first_name: assignment?.first_name || null,
        last_name: assignment?.last_name || null,
        assigned_at: assignment?.created_at || null,
        updated_at: assignment?.updated_at || null,
        // Status
        is_assigned: !!assignment?.user_id,
        is_in_local_db: !!assignment
      };
    });

    console.log(`✅ Enriched ${enrichedPhoneNumbers.length} phone numbers with assignment data`);

    res.status(200).json({
      success: true,
      data: enrichedPhoneNumbers,
      message: "All VAPI phone numbers with assignment info retrieved successfully",
      debug_info: {
        total_vapi_numbers: vapiPhoneNumbers.length,
        total_local_assignments: localPhoneNumbers.length,
        assigned_numbers: enrichedPhoneNumbers.filter(pn => pn.is_assigned).length,
        unassigned_numbers: enrichedPhoneNumbers.filter(pn => !pn.is_assigned).length,
        in_local_db: enrichedPhoneNumbers.filter(pn => pn.is_in_local_db).length,
        not_in_local_db: enrichedPhoneNumbers.filter(pn => !pn.is_in_local_db).length
      }
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch VAPI phone numbers",
      error: error.message
    });
  }
};

// Assign phone number to a user (super admin only)
const assignPhoneNumberToUser = async (req, res) => {
  try {
    const { id } = req.params; // This could be either local DB ID or VAPI ID
    const { user_id, vapi_id } = req.body;
    const currentUserId = req.user?.user?.id || req.user?.id;
    const isSuperAdmin = req.user?.isSuperAdmin || false;

    console.log(`🔄 Assignment request: ID=${id}, user_id=${user_id}, vapi_id=${vapi_id}`);

    // Check if user is super admin
    if (!isSuperAdmin) {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only super admin can assign phone numbers."
      });
    }
    ...
    // If vapi_id is provided, this is a VAPI phone number that might not be in local DB
    if (vapi_id) {
      console.log(`📱 Looking for VAPI phone number: ${vapi_id}`);

      // First, try to find existing local record by VAPI ID
      const existingRecord = await PhoneNumberModel.getPhoneNumberByVapiId(vapi_id);

      if (existingRecord) {
        console.log(`✅ Found existing local record for VAPI ID ${vapi_id}`);
        phoneNumber = existingRecord;
        localId = existingRecord.id;
      } else {
        console.log(`🆕 VAPI phone number not in local DB, fetching from VAPI...`);

        // Fetch phone number details from VAPI
        const vapiResponse = await axios.get(`${VAPI_BASE_URL}/phone-number/${vapi_id}`, {
          headers: {
            Authorization: `Bearer ${VAPI_SECRET_KEY}`,
            "Content-Type": "application/json"
          }
        ...
        const vapiPhoneNumber = vapiResponse.data;
        console.log(`📞 VAPI phone number details:`, vapiPhoneNumber);

        // Create local record for this VAPI phone number
        const phoneNumberRecord = {
          user_id: user_id, // Assign directly to the user
          phone_number_id: vapiPhoneNumber.id,
          org_id: vapiPhoneNumber.orgId || null,
          number: vapiPhoneNumber.number,
          country_code: vapiPhoneNumber.countryCode || null,
          provider: 'vapi',
          type: vapiPhoneNumber.type || 'local',
          capabilities: vapiPhoneNumber.capabilities ? JSON.stringify(vapiPhoneNumber.capabilities) : null,
          sip_uri: vapiPhoneNumber.sipUri || null,
          status: 'active',
          created_by: currentUserId
        };

        const newLocalId = await PhoneNumberModel.createPhoneNumber(phoneNumberRecord);
        console.log(`✅ Created local record with ID ${newLocalId} for VAPI phone number ${vapi_id}`);

        // Get the newly created record
        phoneNumber = await PhoneNumberModel.getPhoneNumberById(newLocalId);
        localId = newLocalId;
      }
    } else {
      // Traditional assignment using local DB ID
      phoneNumber = await PhoneNumberModel.getPhoneNumberById(id);
      if (!phoneNumber) {
    ...
    // Assign phone number to user (or update assignment if already assigned)
    const result = await PhoneNumberModel.assignPhoneNumberToUser(localId, user_id);

    if (result) {
      res.status(200).json({
        success: true,
        message: "Phone number assigned successfully",
        data: { phone_number_id: id, user_id: user_id }
      });
    } else {
      res.status(500).json({
        success: false,
...
// Unassign phone number (super admin only)
const unassignPhoneNumber = async (req, res) => {
  try {
    ...
    if (!isSuperAdmin) {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only super admin can unassign phone numbers."
      });
    }
    ...
    const phoneNumber = await PhoneNumberModel.getPhoneNumberById(id);
    if (!phoneNumber) {
      return res.status(404).json({
        success: false,
        message: "Phone number not found."
    ...
    const result = await PhoneNumberModel.unassignPhoneNumber(id);
...
// Debug function to show all phone number assignments
const debugPhoneNumberAssignments = async (req, res) => {
  try {
    const PhoneNumberModel = require("../../model/PhoneNumberModel/PhoneNumberModel");
    const phoneNumbers = await PhoneNumberModel.getAllPhoneNumbersForAdmin();
    ...
    res.status(200).json({
      success: true,
      data: phoneNumbers,
      message: "Debug: All phone number assignments",
      debug_info: {
        total_count: phoneNumbers.length,
        assigned_count: phoneNumbers.filter(pn => pn.user_id).length,
        unassigned_count: phoneNumbers.filter(pn => !pn.user_id).length
      }
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch debug assignments",
      error: error.message
    });
  }
};

// Debug function to show what phone numbers a user sees
const debugUserPhoneNumbers = async (req, res) => {
  try {
    const currentUserId = req.user?.user?.id || req.user?.id;
    const isSuperAdmin = req.user?.isSuperAdmin || false;

    const PhoneNumberModel = require("../../model/PhoneNumberModel/PhoneNumberModel");
    const userPhoneNumbers = await PhoneNumberModel.getPhoneNumbersByUserId(currentUserId);

    res.status(200).json({
      success: true,
      data: userPhoneNumbers,
      message: "Debug: User's phone numbers",
      debug_info: {
        user_id: currentUserId,
        is_super_admin: isSuperAdmin,
        phone_count: userPhoneNumbers.length,
        phone_number_ids: userPhoneNumbers.map(pn => pn.phone_number_id),
        user_object: req.user
      }
    });
  } catch (error) {
    console.error('Error in debug user numbers:', error);
    res.status(500).json({
      success: false,
      message: "Failed to fetch debug user numbers",
      error: error.message
    });
  }
};
...
module.exports = {
  ...
  unassignPhoneNumber,
  debugPhoneNumberAssignments,
  debugUserPhoneNumbers
};
