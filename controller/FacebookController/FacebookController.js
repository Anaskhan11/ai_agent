...
// Get Facebook connection status for a specific webhook
const getFacebookConnectionStatus = async (req, res) => {
  try {
    ...
    console.log(`🔍 Getting Facebook connection status for webhook: ${webhookId}`);
    ...
    try {
      ...
      console.log(`📋 Database query result:`, { rowsLength: rows.length, webhookId, userId });

      if (rows.length === 0) {
        console.log(`❌ Webhook not found in database: ${webhookId} for user: ${userId}`);
      ...
      console.log(`📋 Raw webhook metadata from database:`, rows[0].metadata);
      ...
      try {
        ...
        } else if (typeof rows[0].metadata === 'object' && rows[0].metadata !== null) {
        ...
        console.error('Error parsing webhook metadata:', e);
        metadata = {};
      }

      // Check if Facebook connection data exists in metadata
      const facebookConnection = metadata.facebookConnection;

      if (facebookConnection && facebookConnection.isConnected) {
        console.log(`✅ Found Facebook connection in webhook metadata:`, {
          isConnected: facebookConnection.isConnected,
          pagesCount: facebookConnection.pages?.length || 0,
          selectedPageId: facebookConnection.selectedPageId
        });
        ...
        return res.status(200).json({
          ...
          data: {
            isConnected: true,
            facebookUser: facebookConnection.facebookUser,
            pages: facebookConnection.pages || [],
            selectedPageId: facebookConnection.selectedPageId || '',
            selectedLeadFormId: facebookConnection.selectedLeadFormId || '',
            leadForms: facebookConnection.leadForms || []
          }
        });
      } else {
        console.log(`❌ No Facebook connection found in webhook metadata for: ${webhookId}`);
        return res.status(200).json({
          success: true,
          data: {
            isConnected: false
    ...
    console.error("Error getting Facebook connection status:", error.message);
    res.status(500).json({
      success: false,
      message: "Failed to get Facebook connection status",
      error: error.message
    });
  }
};
