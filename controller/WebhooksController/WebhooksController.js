...
const createWebhook = async (req, res) => {
  try {
    ...
    try {
      ...
      // Deduct credits for webhook creation (only if not super admin)
      if (req.creditInfo && !req.creditInfo.bypassCredits) {
        try {
          await deductCredits(req, null, () => {});
          console.log('✅ Credits deducted for webhook creation');
        } catch (creditError) {
          console.error('❌ Error deducting credits for webhook creation:', creditError);
          // Note: We don't fail the webhook creation if credit deduction fails
          // as the webhook has already been created successfully
        }
      }
...
const updateWebhook = async (req, res) => {
  try {
    ...
    const { name, description, triggerType, url, isActive, actionCards, facebookConnection } = req.body;
    const userId = req.user?.id;

    console.log('🔄 Updating webhook:', webhookId);
    console.log('📧 Facebook connection data received:', facebookConnection);
    ...
    try {
      ...
      // Add/merge Facebook connection data if provided
      if (facebookConnection) {
        metadata.facebookConnection = facebookConnection;
        console.log('✅ Added Facebook connection to metadata:', facebookConnection);
      }

      console.log('💾 Final metadata to save (merged):', JSON.stringify(metadata, null, 2));
...
const listWebhooks = async (req, res) => {
  try {
    ...
    try {
      ...
      let query, params;
      if (isSuperAdmin) {
        // Super admin can see all webhooks with user information
        query = `
          SELECT w.*, u.username, u.email, u.first_name, u.last_name
          FROM webhooks w
          LEFT JOIN users u ON w.user_id = u.id
          ORDER BY w.created_at DESC
        `;
        params = [];
        console.log(`🔑 Super admin ${userId} requesting all webhooks`);
      } else {
        // Regular users can only see their own webhooks
        query = `SELECT * FROM webhooks WHERE user_id = ? ORDER BY created_at DESC`;
        params = [userId];
        console.log(`👤 Regular user ${userId} requesting their webhooks`);
      }
      ...
      if (isSuperAdmin) {
        console.log(`🔑 Found ${webhooks.length} total webhooks for super admin ${userId}`);
        if (webhooks.length > 0) {
          console.log('Sample webhook structure (super admin view):', {
            id: webhooks[0].id,
            webhook_id: webhooks[0].webhook_id,
            name: webhooks[0].name,
            user_id: webhooks[0].user_id,
            username: webhooks[0].username,
            hasWebhookIdField
          });
        }
      } else {
  ...
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO contacts (
        email, fullName, phoneNumber, phone2, listId, createdAt,
      ...
      [
        ...
        webhookData.phone2 || webhookData.secondaryPhone || null,
        listId,
        webhookData.patient_lead_source || webhookData.leadSource || 'webhook',
        webhookData.banned || false,
        webhookData.patient_lead_owner || webhookData.leadOwner || null,
        webhookData.patient_lead_name || webhookData.leadName || null,
        webhookData.date_of_birth || webhookData.dateOfBirth || webhookData.dob || null,
        webhookData.age || null,
        webhookData.height || null,
        webhookData.weight_lbs || webhookData.weight || null,
        webhookData.habits || null,
        webhookData.medications || null,
        webhookData.diagnosis || null,
        webhookData.surgeries || null,
        webhookData.status || 'new',
        webhookData.qualified_status || 'pending',
        webhookData.dnq || false,
        webhookData.not_interested_reasons || null,
        null, // created_by - will be set to null for webhook entries
        null  // modified_by - will be set to null for webhook entries
      ]
    );
...
const addContactToList = async (userId, listId, contactInfo) => {
  try {
    ...
    try {
      ...
      // Add new contact with enhanced patient fields
      const [result] = await connection.execute(
        `INSERT INTO contacts (
          fullName, email, phoneNumber, phone2, listId, createdAt,
          patient_lead_source, banned, patient_lead_owner, patient_lead_name,
          date_of_birth, age, height, weight_lbs, habits, medications,
          diagnosis, surgeries, status, qualified_status, dnq,
          not_interested_reasons, created_by, modified_by
        ...
        [
          ...
          listId,
          contactInfo.patient_lead_source || 'facebook',
          contactInfo.banned || false,
          contactInfo.patient_lead_owner || null,
          contactInfo.patient_lead_name || null,
          contactInfo.date_of_birth || null,
          contactInfo.age || null,
          contactInfo.height || null,
          contactInfo.weight_lbs || null,
          contactInfo.habits || null,
          contactInfo.medications || null,
          contactInfo.diagnosis || null,
          contactInfo.surgeries || null,
          contactInfo.status || 'new',
          contactInfo.qualified_status || 'pending',
          contactInfo.dnq || false,
          contactInfo.not_interested_reasons || null,
          null, // created_by - will be set to null for Facebook webhook entries
          null  // modified_by - will be set to null for Facebook webhook entries
        ]
      );
