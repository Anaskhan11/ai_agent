...
class VapiService {
  ...
  async createCredential(credentialData) {
    try {
      ...
      console.log('Full credential data being sent:', JSON.stringify(credentialData, null, 2));
  ...
  // Import Twilio phone number in Vapi
  async importTwilioPhoneNumber(importData) {
    try {
      console.log('Importing Twilio phone number in Vapi:', importData.twilioPhoneNumber);
      console.log('Full import data being sent:', JSON.stringify(importData, null, 2));

      const response = await this.client.post('/phone-number/import/twilio', importData);

      console.log('Vapi Twilio phone number imported successfully:', response.data.id);
      return response.data;
    } catch (error) {
      console.error('Error importing Twilio phone number in Vapi:', error.response?.data || error.message);
      console.error('Full error response:', JSON.stringify(error.response?.data, null, 2));
      throw new Error(`Vapi Twilio Import API Error: ${error.response?.data?.message || error.message}`);
    }
  }
