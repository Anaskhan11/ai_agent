...
const getAssistants = async (req, res) => {
  ...
  // SUPER ADMIN OPTIMIZATION: Remove pagination for super admin to show all assistants
  let page, limit;
  if (isSuperAdmin) {
    console.log("👑 Super admin detected - fetching ALL assistants without pagination");
    page = 1;
    limit = 10000; // Large limit to get all assistants
  } else {
    page = !isNaN(pageParam) && pageParam > 0 ? pageParam : 1;
    limit = !isNaN(limitParam) && limitParam > 0 ? limitParam : 10;
  }
  ...
  try {
    ...
    const totalPages = isSuperAdmin ? 1 : Math.ceil(totalAssistant / limit); // Super admin gets all in one page
    const currentPage = isSuperAdmin ? 1 : (page > totalPages && totalPages > 0 ? totalPages : page);

    console.log(`📊 Returning ${syncedAssistants.length} assistants (Super Admin: ${isSuperAdmin}, Total: ${totalAssistant})`);
    ...
    res.status(200).json({
      ...
      totalPages,
      syncVerified: syncCheck,
      paginationDisabled: isSuperAdmin, // Indicate if pagination is disabled
      message: isSuperAdmin ? `Super admin view: showing all ${totalAssistant} assistants` : undefined
    });
  } catch (error) {
...
const getSyncedAssistants = async (req, res) => {
  ...
  const limitParam = parseInt(req.query.limit);
  const search = req.query.search || "";
  const skipSync = req.query.skipSync === 'true'; // Option to skip VAPI sync check for performance
  ...
  let page, limit;
  if (isSuperAdmin) {
    console.log("👑 Super admin detected - fetching ALL synced assistants without pagination");
    page = 1;
    limit = 50000; // Very large limit to get all assistants for super admin
  } else {
    page = !isNaN(pageParam) && pageParam > 0 ? pageParam : 1;
    limit = !isNaN(limitParam) && limitParam > 0 ? limitParam : 10;
  }
  ...
  try {
    ...
    console.log('  - Skip Sync Check:', skipSync);
    console.log('  - Pagination Disabled:', isSuperAdmin);

    // Super admin can see all assistants, regular users only see their own
    const userId = isSuperAdmin ? null : currentUserId;

    // Get assistants from local database (all for super admin, paginated for regular users)
    const { data: allLocalAssistants } = await AssistantModel.getAssistants(page, limit, search, userId);
    ...
    if (!allLocalAssistants || allLocalAssistants.length === 0) {
      ...
      return res.status(200).json({
        ...
        currentPage: 1,
        totalPages: 0,
        syncVerified: !skipSync,
        paginationDisabled: isSuperAdmin,
        message: "No assistants created yet"
      });
    }

    // If skipSync is true, return local data without VAPI verification (for performance)
    if (skipSync) {
      console.log(`⚡ Returning ${allLocalAssistants.length} local assistants without VAPI sync check (Super Admin: ${isSuperAdmin})`);
      return res.status(200).json({
        data: allLocalAssistants.map(assistant => ({
          ...assistant,
          syncStatus: 'local_only',
          lastSyncCheck: null
        })),
        totalAssistant: allLocalAssistants.length,
        currentPage: 1,
        totalPages: 1,
        syncVerified: false,
        paginationDisabled: isSuperAdmin,
        message: isSuperAdmin
          ? `Super admin view: showing all ${allLocalAssistants.length} assistants (sync check skipped for performance)`
          : `Found ${allLocalAssistants.length} local assistants (sync check skipped for performance)`
      });
    }
    ...
    console.log(`✅ Found ${syncedAssistants.length} fully synced assistants (Super Admin: ${isSuperAdmin})`);

    // Apply pagination to synced results (or return all for super admin)
    const totalSynced = syncedAssistants.length;
    let finalResults, finalTotalPages, finalCurrentPage;

    if (isSuperAdmin) {
      // Super admin gets all results without pagination
      finalResults = syncedAssistants;
      finalTotalPages = 1;
      finalCurrentPage = 1;
    } else {
      // Regular users get paginated results
      const totalPages = Math.ceil(totalSynced / limit);
      const currentPage = page > totalPages && totalPages > 0 ? totalPages : page;
      const offset = (currentPage - 1) * limit;
      finalResults = syncedAssistants.slice(offset, offset + limit);
    ...
    res.status(200).json({
      ...
      paginationDisabled: isSuperAdmin,
      message: isSuperAdmin
        ? `Super admin view: showing all ${totalSynced} assistants that exist in both local DB and VAPI`
        : `Found ${totalSynced} assistants that exist in both local DB and VAPI`
    });
  } catch (error) {
