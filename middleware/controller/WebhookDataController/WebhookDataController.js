...
const processWebhookWorkflow = async (webhookId, dataId, rawData) => {
  try {
    ...
    try {
      ...
      try {
        const metaObj = typeof webhook.metadata === 'string' ? JSON.parse(webhook.metadata || '{}') : (webhook.metadata || {});
        const actionCards = Array.isArray(metaObj.actionCards) ? metaObj.actionCards : [];
        const gmailCard = actionCards.find(c => c?.selectedApp?.id === 'gmail-service');
      ...
      if (gmailConnected) {
        console.log(`✅ Gmail service is connected via action card; sending notification...`);
      ...
      // Only send Gmail notifications if Gmail action is connected
      if (gmailConnected) {
        console.log(`📧 Sending webhook data to connected Gmail accounts...`);
        try {
          await WebhookGmailNotificationService.sendWebhookDataToConnectedGmails(
            rawData,
            webhook,
            webhookId
          );
          console.log(`✅ Webhook data sent to connected Gmail accounts successfully`);
        } catch (gmailError) {
          console.error(`❌ Error sending webhook data to Gmail accounts:`, gmailError);
          // Don't fail the webhook processing if Gmail notification fails
        }
      } else {
        console.log('✉️ Gmail service not connected; skipping Gmail notifications');
      }
...
const maybeTriggerOutboundCampaignFromWebhook = async (webhook, rawData, workflowConfig, structuredData) => {
  try {
    ...
    if (!phoneNumberId) {
      console.log('❌ Campaign config missing phoneNumberId and no DEFAULT_VAPI_PHONE_NUMBER_ID; skipping');
      return;
    }
