...
module.exports = {
  ...
  facebookOAuthCallback: async (req, res) => {
    try {
      ...
      // Add headers to bypass ngrok warning and improve compatibility
      ...
      // Handle errors by returning HTML that will close the window and notify the opener
      if (error || !code || !state) {
        const errorMessage = error || 'Missing required parameters';
        const html = `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body>
            <div class="container">
              ...
              <div class="message">Facebook Connection Failed</div>
            ...
            <script>
              try {
                if (window.opener && !window.opener.closed) {
                  ...
                  setTimeout(() => {
                    window.location.href = '${frontendOrigin}/#/webhooks?facebook_error=${encodeURIComponent(errorMessage)}';
                  }, 3000);
                }
              } catch (error) {
                setTimeout(() => {
        ...
        const html = `
          ...
          <body>
            <div class="container">
              <div class="error-icon">❌</div>
              <div class="message">Facebook Connection Failed</div>
              <div class="sub-message">Invalid authentication state. This window will close automatically...</div>
            </div>
            <script>
              try {
                if (window.opener && !window.opener.closed) {
                  ...
                  setTimeout(() => {
                    window.location.href = '${frontendOrigin}/#/webhooks?facebook_error=invalid_state';
                  }, 3000);
                }
              } catch (error) {
                setTimeout(() => {
      ...
      console.log('Facebook token response:', { access_token: access_token ? 'present' : 'missing', expires_in });
      ...
      const facebookUser = userResponse.data;
      console.log('Facebook user data:', facebookUser);

      // Validate required data before database insertion
      if (!facebookUser.id || !access_token || !userId) {
        throw new Error(`Missing required Facebook user data: userId=${userId}, facebookUser.id=${facebookUser.id}, access_token=${access_token ? 'present' : 'missing'}`);
      }
      ...
      const dbParams = [
        ...
        facebookUser.id,
        access_token,
        expires_in || 3600, // Default to 1 hour if expires_in is undefined
        facebookUser.name || '',
        facebookUser.email || ''
      ];

      console.log('Database parameters:', dbParams.map((param, index) => `${index}: ${param === undefined ? 'UNDEFINED' : typeof param === 'string' ? `"${param}"` : param}`));
      ...
      // Send HTML with JavaScript to close the popup and notify parent window
      const html = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <div class="message">Facebook Connected Successfully!</div>
            <div class="sub-message">This window will close automatically...</div>
          </div>
          <script>
            try {
              if (window.opener && !window.opener.closed) {
                ...
                window.opener.postMessage({
                  ...
                  data: {
                    isConnected: true,
                    facebookUser: {
                      name: '${(facebookUser.name || '').replace(/'/g, "\\'")}',
                      email: '${(facebookUser.email || '').replace(/'/g, "\\'")}',
                      id: '${(facebookUser.id || '').replace(/'/g, "\\'")}'
                    }
                  }
                }, '*');
                ...
                setTimeout(() => {
                  window.location.href = '${frontendOrigin}/#/webhooks?facebook_success=true';
                }, 2000);
              }
            } catch (error) {
      ...
      const errorMessage = (err.response?.data?.error_description || err.response?.data?.error || err.message || 'Unknown error').replace(/'/g, "\\'");
      const frontendOrigin = (process.env.FRONTEND_URL || '').replace(/\/$/, '');
      const html = `
        <!DOCTYPE html>
        <html>
