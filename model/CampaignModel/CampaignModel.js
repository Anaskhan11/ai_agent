...
const getCampaignsByUserId = async (userId) => {
  return await retryDbOperation(async () => {
    ...
    return rows.map(row => ({
      ...row,
      contact_list_ids: safeJsonParse(row.contact_list_ids, []),
      schedule_config: safeJsonParse(row.schedule_config, {}),
      targeting_rules: safeJsonParse(row.targeting_rules, {}),
      call_settings: safeJsonParse(row.call_settings, {}),
      performance_metrics: safeJsonParse(row.performance_metrics, {}),
